...
export type OpenFetchClient<Paths> = <ReqT extends Extract<keyof Paths, string>, Method extends Extract<keyof Paths[ReqT], string> | Uppercase<Extract<keyof Paths[ReqT], string>>, LowercasedMethod extends Lowercase<Method> extends keyof Paths[ReqT] ? Lowercase<Method> : never, DefaultMethod extends 'get' extends LowercasedMethod ? 'get' : LowercasedMethod, ResT = FetchResponseData<Paths[ReqT][DefaultMethod]>>(url: ReqT, options?: OpenFetchOptions<Method, LowercasedMethod, Paths[ReqT]>) => Promise<ResT>;
export type UseOpenFetchClient<Paths, Lazy extends boolean = false> = <ReqT extends Extract<keyof Paths, string>, Method extends Extract<keyof Paths[ReqT], string> | Uppercase<Extract<keyof Paths[ReqT], string>>, LowercasedMethod extends Lowercase<Method> extends keyof Paths[ReqT] ? Lowercase<Method> : never, DefaultMethod extends 'get' extends LowercasedMethod ? 'get' : LowercasedMethod, ResT = FetchResponseData<Paths[ReqT][DefaultMethod]>, ErrorT = FetchResponseError<Paths[ReqT][DefaultMethod]>, DataT = ResT, PickKeys extends KeysOf<DataT> = KeysOf<DataT>, DefaultT = null>(url: ReqT | (() => ReqT), options?: Lazy extends true ? Omit<UseOpenFetchOptions<Method, LowercasedMethod, Paths[ReqT], ResT, DataT, PickKeys, DefaultT>, 'lazy'> : UseOpenFetchOptions<Method, LowercasedMethod, Paths[ReqT], ResT, DataT, PickKeys, DefaultT>, autoKey?: string) => AsyncData<PickFrom<DataT, PickKeys> | DefaultT, ErrorT | null>;
export declare function createOpenFetch(options: FetchOptions | ((options: FetchOptions) => FetchOptions)): (url: string, opts: any) => Promise<any>;
export declare function createUseOpenFetch<Paths, Lazy extends boolean = false>(client: $Fetch | OpenFetchClientName, lazy?: Lazy): UseOpenFetchClient<Paths, Lazy>;
